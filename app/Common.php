...
if(
    !function_exists('manage_path')
){
    function manage_path(){
        return $_SERVER['HTTP_HOST'] == 'localhost' ? '' : '/var/www/html/ndb-fs/uploads/';
    }
}

if(
    !function_exists('manage_path_fc')
){
    function manage_path_fc(){
        return $_SERVER['HTTP_HOST'] == 'localhost' ? FCPATH : '/var/www/html/ndb-fs/uploads/';
    }
}


if(
    !function_exists('access_path')
){
    function access_path($path = ''){
        return $_SERVER['HTTP_HOST'] == 'localhost' ? base_url($path) : 'https://www.drive.nepaldatabase.com/'.$path;
    }
}
...
if (!function_exists('getFavicon')) {
    function getFavicon()
    {
        ...
        if (!empty($generalSettings)) {
            if (!empty($generalSettings->favicon) && file_exists( manage_path_fc(). 'uploads/' . $generalSettings->favicon)) {
                return access_path('uploads/'.$generalSettings->favicon);
            }
            return access_path("assets/img/favicon.png");
        }
        return access_path("assets/img/favicon.png");
    }
}
...
if (!function_exists('getLogo')) {
    function getLogo()
    {
        $generalSettings = Globals::$generalSettings;
        if (!empty($generalSettings)) {
            if (!empty($generalSettings->logo) && file_exists(manage_path_fc().'uploads/' . $generalSettings->logo)) {
                return access_path('uploads/'.$generalSettings->logo);
            }
            return access_path("assets/img/logo.svg");
        }
        return access_path("assets/img/logo.svg");
    }
}
...
if (!function_exists('getLogoFooter')) {
    function getLogoFooter()
    {
        $generalSettings = Globals::$generalSettings;
        if (!empty($generalSettings)) {
            if (!empty($generalSettings->logo_footer) && file_exists(manage_path_fc() .'uploads/'. $generalSettings->logo_footer)) {
                return access_path('uploads/'.$generalSettings->logo_footer);
            }
            return access_path("assets/img/logo-footer.svg");
        }
        return access_path("assets/img/logo-footer.svg");
    }
}
...
if (!function_exists('getLogoEmail')) {
    function getLogoEmail()
    {
        $generalSettings = Globals::$generalSettings;
        if (!empty($generalSettings)) {
            if (!empty($generalSettings->logo_email) && file_exists(manage_path_fc() .'uploads/' . $generalSettings->logo_email)) {
                return access_path('uploads/'.$generalSettings->logo_email);
            }
            return access_path("assets/img/logo.png");
        }
        return access_path("assets/img/logo.png");
    }
}
...
if (!function_exists('getUserAvatar')) {
    ...
    {
        if (!empty($avatarPath)) {
            if (file_exists(manage_path_fc() .'uploads/' . $avatarPath)) {
                return access_path('uploads/'.$avatarPath);
            }
            return $avatarPath;
        }
        return access_path("assets/img/user.png");
    }
}
    ...
    function getPostImage($post, $imageSize)
    {
        if (!empty($post)) {
            if (!empty($post->image_url)) {
                ...
                return access_path('uploads/'.$path);
            }
        }
    }
}
...
if (!function_exists('setPostMetaTags')) {
    function setPostMetaTags($post, $postTags, $data)
    {
        ...
        $data['ogType'] = 'article';
        $data['ogImage'] = 'https://drive.nepaldatabase.com/uploads/'.($post->image_big);
        if (!empty($post->image_url)) {
            $data['ogImage'] = 'https://drive.nepaldatabase.com/uploads/'.$post->image_url;
        }
        $data['ogWidth'] = '750';
        $data['ogHeight'] = '422';
